...
$fh = fopen('historic_fires.csv', 'wb');
fputcsv($fh, array('name','size', 'lat', 'lng', 'cause', 'date', 'fuels', 'personnel', 'contained', 'location', 'events', 'weather', 'link', 'last_update'));

process_page($html, $site_base, $fh);

$unique_links = range(10, 80, 10);
...
if(copy('historic_fires.csv', 'data/historic_fires.csv')) {
    echo "Fires Updated\n";
} else {
...
function process_page($html, $site_base, $fh = false) {
    ...
    foreach($fires as $fire) {
        ...
        if($fire_size > 0 && in_array($state, $states)) { //
            $data = array();
...
function process_fire($html, $data, $full_link) {
    ...
    foreach($details as $detail) {
        ...
        foreach($infos as $info) {
            ...
            } elseif($field == 'Current as of') {
                $last_date = $info->find('td', 0);
                $last = trim($last_date->plaintext);
                $data[13] = $last;
            }
        }
    }
